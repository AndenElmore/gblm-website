    ...
    <section className="pt-48 pb-24 bg-white border-t border-gray-200">
      <div className="container mx-auto px-5 max-w-5xl">
        <div className="text-center mb-16">
          <h2 className="text-5xl md:text-7xl font-bold font-montserrat text-[#514d4c] mb-6 drop-shadow-sm uppercase">Frequently Asked Questions</h2>
        ...
        <div className="space-y-6">
          {faqs.map((faq, index) => {
            ...
            return (
              <div key={index} className={`bg-white rounded-xl shadow-md border overflow-hidden transition-all duration-300 ${isOpen ? 'border-[#514d4c] shadow-lg ring-1 ring-[#514d4c]' : 'border-gray-200 hover:border-[#9cc0ed]'}`}>
                <button 
                  className="w-full text-left px-8 py-6 flex justify-between items-center focus:outline-none group"
                  onClick={() => toggleFAQ(index)}
                >
                  <span className={`font-bold text-xl md:text-2xl transition-colors duration-300 ${isOpen ? 'text-[#514d4c]' : 'text-gray-900'}`}>{faq.question}</span>
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 ${isOpen ? 'bg-[#514d4c] text-white rotate-180' : 'bg-gray-100 text-[#9cc0ed] group-hover:bg-[#9cc0ed] group-hover:text-white'}`}>
